...
- (void)_initialisation {
    ...
    _maxSelectedCount = 10;
    _showSelectedCount = NO;
...
- (void)performLayout {
    ...
    if (_showSelectedCount) {
        UIBarButtonItem *selectedCount = [[UIBarButtonItem alloc] initWithTitle:@"选择(0)" style:UIBarButtonItemStylePlain target:self action:nil];
        [items addObject:selectedCount];
    }
...
- (void)setPhotoSelected:(BOOL)selected atIndex:(NSUInteger)index {
    ...
    if (_showSelectedCount) {
        if ([self.delegate respondsToSelector:@selector(selectedCountPhotoBrowser:)]) {
            NSInteger count = [self.delegate selectedCountPhotoBrowser:self];
            NSArray* items = [_toolbar items];
            UIBarButtonItem * countLabel = [items lastObject];
            countLabel.title = [NSString stringWithFormat:@"选择(%ld)",(long)count];
        }
    }
}
...
- (void)selectedButtonTapped:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(selectedCountPhotoBrowser:)]) {
        NSInteger count = [self.delegate selectedCountPhotoBrowser:self];
        if (count >= self.maxSelectedCount && selectedButton.selected == NO) {
            
            if ([self.delegate respondsToSelector:@selector(reachMaxSelectedCountPhotoBrowser:)]) {
                [self.delegate reachMaxSelectedCountPhotoBrowser:self];
            }
